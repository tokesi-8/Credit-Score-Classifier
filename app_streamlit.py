...
# Model loading 
@st.cache_resource #fitur dari streamlit untuk simpan model di RAM Streamlit
def load_model():
...
def predict(model, values: dict):
    df_input = pd.DataFrame([{col: values[col] for col in ALL_MODEL_COLS}])
    pred = model.predict(df_input)[0]
    proba = model.predict_proba(df_input)[0]
    label = TARGET_NAMES[pred]
    proba_dict = {TARGET_NAMES[i]: p for i, p in enumerate(proba)}
...
with st.sidebar:
    ...
    if model is not None:
        ...
        st.caption("`models/model_lightgbm.pkl`")
    else:
...
if model is None:
    st.warning("Fix error dari load model sebelum ke tahap berikutnya.")
    st.stop()
...
if submitted:
    ...
    try:
        label, proba = predict(model, values)
